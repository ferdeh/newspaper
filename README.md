...
Celery refreshes `analytics_daily` rather than forcing dashboard endpoints to scan raw article/post tables. Available interfaces include Overview, Situation Map, Incident list/detail, News, Geographic/Product/Event Intelligence, **TBBM Incident Exposure**, Reported MT Accidents, Alerts, System Monitor, Settings, and TikTok Discovery with Overview, public signals, Early Warning, keyword, manual search, video, run, and provider-settings views.
