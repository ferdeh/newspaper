...
async function proxy(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  if (!path.length || path.some((part) => !/^[a-zA-Z0-9_-]+$/.test(part))) {
    return NextResponse.json({ detail: "Path TikTok Discovery tidak valid." }, { status: 400 });
  }
  const sourceUrl = new URL(request.url);
  const target = `${internalApi}/tiktok/${path.join("/")}${sourceUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json({ detail: "Server tidak dapat menghubungi layanan TikTok Discovery." }, { status: 502 });
  }
}
