...
async function proxy(request: Request, context: { params: Promise<{ path: string[] }> }) {
  if (!sameOrigin(request)) return NextResponse.json({ detail: "Permintaan lintas-origin ditolak." }, { status: 403 });
  const { path } = await context.params;
  if (!path.length || path.some((part) => !/^[a-zA-Z0-9_-]+$/.test(part))) {
    return NextResponse.json({ detail: "Path notification tidak valid." }, { status: 400 });
  }
  ...
  const target = `${internalApi}/${path.join("/")}${incoming.search}`;
  ...
  try {
    ...
    return NextResponse.json({ detail: "Server tidak dapat menghubungi layanan Notification." }, { status: 502 });
  }
}
