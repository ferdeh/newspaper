...
export async function proxyNewsKeyword(request: Request, keywordId?: string) {
  if (!requestIsSameOrigin(request)) {
    return NextResponse.json({ detail: "Permintaan lintas-origin ditolak." }, { status: 403 });
  }
  if (keywordId !== undefined && !/^\d+$/.test(keywordId)) {
    return NextResponse.json({ detail: "ID news keyword tidak valid." }, { status: 400 });
  }
  ...
  const target = `${internalApi}/admin/news-keywords${keywordId === undefined ? "" : `/${keywordId}`}`;
  ...
  try {
    ...
    return NextResponse.json({ detail: "Server tidak dapat menghubungi layanan News Keywords." }, { status: 502 });
  }
}
